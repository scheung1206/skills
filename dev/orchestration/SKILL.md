...
When Step 1 yields **seam-independent** pieces (each owns distinct files / a distinct worktree / a distinct interface with no read-write overlap), execute them as a **parallel fan-out** rather than serially:
- **Fan-out (background, concurrent).** Dispatch the pieces as background leaf sub-agents (via `delegate_task` with a `tasks` list). They run concurrently in isolated terminals/sessions; results consolidate on completion. The orchestrator stays alive to do fan-in. Concurrency is capped by `delegation.max_concurrent_children` (default **3**) — raise it only when you genuinely have more independent seams; forcing N agents on fewer seams just makes them wait or collide.
...
- **Depth = 1.** Sub-agents are leaves (your `max_spawn_depth=1`); they cannot spawn further agents. One orchestrator → N implementers is the correct ceiling; deeper nesting is where coordination rots.
...
- DON'T raise `max_concurrent_children` beyond the number of genuinely independent seams you have; forcing N agents on fewer seams just makes them wait or collide.
