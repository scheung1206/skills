...
## v1.4.0 — 2026-08-15 (PENDING merge)
- CLARIFIED: the "posted review loop" makes every turn's MR-comment posting explicit and mandatory (Step 7). The review is a loop, not a one-shot; the reviewer verdict, the implementer's fix-response, and the re-review verdict each MUST be posted as an MR comment (R5) — a verdict surviving only in a local file or chat is not a review-of-record. Fix-response must state *what changed + how verified* per finding (with real exit codes), not "fixed per review"; re-review posts a new comment. Added the GitHub `422: cannot approve your own pull request` limitation: when the PR author is the owner's account and the reviewer has no separate GitHub identity, the posted MR comment still stands as the review-of-record and the owner clicks approve.
- DECISION: port the posting discipline into the canonical v1.3.0 SKILL.md (Step 7 + Pitfalls) rather than overwriting it with the stale 1000-line profile copy. The v1.3.0 restructure (R1–R9, verify.py gate) is the active lineage; the profile copy is a divergence to be reconciled, not reverted to.
- REJECTED: pushing the profile's 1000-line `orchestration/orchestrator/SKILL.md` v2.0.0 (references the retired "Didiking" persona, drifted from R1–R9) as the canonical skill — that would undo the v1.1→v1.3 leaner restructure the owner merged via #3/#4/#5.
