...
name: tdd-gate
tier: dev
description: Use when implementation must wait for the owner to review and approve a runnable test or explicit assertion list committed to the branch.
...
5. Present the branch and runnable command to the owner for review.
6. Wait for the owner's explicit green-light; do not begin implementation while the TDD is unapproved.
...
- DON'T change reviewed assertions during implementation without returning to the owner for review.
- DON'T treat a passing test that never failed for the missing behavior as proof of coverage.

## Verify
- Assert from branch history that the TDD commit precedes the implementation commit, the owner approved the TDD before implementation began, and the documented test command exits zero after implementation.
